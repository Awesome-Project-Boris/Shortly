...
def enable_cors_for_resource(api_client, rest_api_id, resource_id):
    ...
    # Ensure MethodResponse for OPTIONS declares all CORS headers
    try:
        api_client.put_method_response(
            ...
            responseParameters={
                f'method.response.header.{k}': False for k in CORS_HEADERS
            }
        )
    except api_client.exceptions.ConflictException:
        pass

    # Ensure IntegrationResponse for OPTIONS includes CORS headers and a response template
    try:
        api_client.put_integration_response(
            ...
            statusCode='200',
            responseParameters={
                f'method.response.header.{k}': v for k, v in CORS_HEADERS.items()
            },
    ...
    try:
        resource = api_client.get_resource(
            restApiId=rest_api_id,
            resourceId=resource_id
        )
    ...
    for method_name in resource.get('resourceMethods', {}):
        ...
        # First, retrieve existing integration to detect proxy vs non-proxy
        integ = api_client.get_integration(
            restApiId=rest_api_id,
            resourceId=resource_id,
            httpMethod=method_name
        )
        is_proxy = integ.get('type') == 'AWS_PROXY' or integ.get('passthroughBehavior') == 'WHEN_NO_MATCH'
        ...
        except (api_client.exceptions.ConflictException, api_client.exceptions.NotFoundException):
            pass

        # For non-proxy integrations, map the header; for proxy, skip mapping
        if not is_proxy:
            try:
                api_client.put_integration_response(
                    restApiId=rest_api_id,
...
def main():
    parser = argparse.ArgumentParser(
        description='Enable CORS on all resources of an existing API Gateway REST API'
    )
    parser.add_argument(
        '--api-id', required=True, help='The ID of the REST API'
    )
    parser.add_argument(
        '--region', default='us-east-1', help='AWS region of the REST API'
    )
    parser.add_argument(
        '--stage', required=True, help='Stage name to redeploy after enabling CORS'
    )
